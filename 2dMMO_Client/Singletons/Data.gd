...
func _ready() -> void:
	...
	var CraftingRecipesFile = FileAccess.open("res://Data/CraftingRecipes.json", FileAccess.READ)
	CraftingRecipes = JSON.parse_string(CraftingRecipesFile.get_as_text())
...
func ItemFromTilesetId(_tileset_id: int, _block_type: String) -> Dictionary:
	for item in ItemData.values():
		if item.TYPE == _block_type and item.TILESET_ID == _tileset_id:
			return item
	return {TILESET_ID = -1, AUTOTILE = "NONE"}
